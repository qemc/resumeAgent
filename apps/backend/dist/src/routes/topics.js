...
export async function topicRoutes(app) {
    ...
    app.get('/topics/:careerPath/:lang', { onRequest: [app.auth] }, async (req) => {
        ...
        const items = await db.query.topics.findMany({
            where: and(eq(topics.user_id, req.user.id), eq(topics.career_path_id, careerPath), eq(topics.resume_lang, lang))
        });
        return items;
    });
    ...
    app.patch('/topics/:careerPath/:lang/:experience/:id', { onRequest: [app.auth] }, async (req, reply) => {
        ...
        const existingItem = await db.query.topics.findFirst({
            where: and(eq(topics.user_id, req.user.id), eq(topics.experience_id, experience), eq(topics.resume_lang, lang), eq(topics.career_path_id, careerPath), eq(topics.id, parsedIncomingItem.data.id))
        });
        if (!existingItem)
        ...
        await db.update(topics)
            ...
            .where(and(eq(topics.id, parsedIncomingItem.data.id), eq(topics.user_id, req.user.id))); // To the correct row
        return updated;
    });
    ...
    app.post('/topics/generate_all/:careerPath/:lang/:experience', { onRequest: app.auth }, async (req, reply) => {
        ...
        try {
            ...
            const topicItems = newTopics.careerPathTopics.map((item) => {
                return {
                    user_id: req.user.id,
                    resume_lang: lang,
            ...
            await db.delete(topics).where(and(eq(topics.user_id, req.user.id), eq(topics.career_path_id, careerPath), eq(topics.experience_id, experience), eq(topics.resume_lang, lang)));
    ...
    app.post('/topics/generate_single/:careerPath/:lang/:experience', { onRequest: app.auth }, async (req, reply) => {
        ...
        const topicData = await db.query.topics.findFirst({
            where: and(eq(topics.id, parsedBody.data.id), eq(topics.user_id, req.user.id))
        });
        if (!topicData)
            throw new AppError(ERRORS.NOT_FOUND);
        const careerPathData = await db.query.careerPaths.findFirst({
            where: and(eq(careerPaths.id, careerPath), eq(careerPaths.user_id, req.user.id))
        });
        if (!careerPathData)
            throw new AppError(ERRORS.NOT_FOUND);
        const careerPathAdjusted = {
            name: careerPathData.name,
            description: careerPathData.description
        };
        trackRegenerate(req.user.id, parsedBody.data.id);
        try {
            const newTopic = await generateSingleTopic(careerPathAdjusted, topicData.topic_text.preTopic, lang, parsedBody.data.userHint, parsedBody.data.topic);
            if (!newTopic)
                throw new AppError(ERRORS.AI_ERROR);
            const updated = {
                ...topicData,
    ...
    app.delete('/topics/:careerPath/:lang/:experience/:id', { onRequest: [app.auth] }, async (req, reply) => {
        ...
        const deleted = await db.delete(topics)
            .where(and(eq(topics.id, id), eq(topics.user_id, req.user.id), eq(topics.career_path_id, careerPath), eq(topics.experience_id, experience), eq(topics.resume_lang, lang)))
            .returning();
        if (deleted.length === 0)
            throw new AppError(ERRORS.NOT_FOUND);
