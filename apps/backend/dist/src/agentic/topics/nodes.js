...
import { getAiEnhancedExperience, getCareerPath, getExperience, updateAiEnhanceLastUpdate } from '../utils';
import { unifyPromptEn, unifyPromptPl, unify_strategy_and_reasoning_en, unify_strategy_and_reasoning_pl, refined_topic_en, refined_topic_pl, original_id_en, original_id_pl, checkEnhanceAccuracyPromptEn, checkEnhanceAccuracyPromptPl, check_update_en, check_update_pl } from './prompts';
import { oai5nano, oai5_1 } from '../models';
...
export async function generateTopics(state) {
...
export async function unifyText(state) {
    const lang = state.resumeLang;
    ...
    if (result.topics.length !== state.careerPathTopics.length)
        throw new AppError(ERRORS.AI_ERROR, `Unify node error: returned topics list length is not equal to the initial topics list length. Initial topics list length: ${state.careerPathTopics.length}, Lenght of list returned by LLM: ${result.topics.length}`);
